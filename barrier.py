...
class Barriers(Sprite):
    def __init__(self, game):
        super().__init__()
        self.barriers = [Barrier(game=game, x=x, y=840) for x in range(250, 1050, 200)]
        # slower way
...
class Barrier(Sprite):
    ...
    block_images = [pg.image.load(f"images/block{x}.png") for x in range(4, 0, -1)]
    block_topL_images = [pg.image.load(f"images/blockTopL{x}.png") for x in range(4, 0, -1)]
    block_topR_images = [pg.image.load(f"images/blockTopR{x}.png") for x in range(4, 0, -1)]
