...
def check_hs_button(stats, button_hs, mouse_x, mouse_y):
    if button_hs.rect.collidepoint(mouse_x, mouse_y):
      with open('high_scores.txt') as f:
          for line in f:
              pass
          last_line = line
          print(last_line)
def check_events(stats, play_button, ship, sound,game,button_hs):
    ...
    for event in pg.event.get():
        if event.type == pg.QUIT: game.finished=True
            ...
            check_hs_button(stats=stats, button_hs=button_hs, mouse_x=mouse_x, mouse_y=mouse_y)
